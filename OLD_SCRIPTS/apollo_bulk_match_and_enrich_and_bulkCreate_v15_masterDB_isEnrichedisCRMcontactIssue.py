...
def run_bulk_create_crm(crm_df):
    ...
    for b_idx, batch in enumerate(batches, 1):
        ...
        try:
            ...
            created = resp.get("created_contacts", [])
            existing = resp.get("existing_contacts", [])

            total_created  += created
            total_existing += existing

            # 🔥 IMPORTANT: capture IDs
            for c in created:
                if c.get("id"):
...
def main():
    ...
    if not person_ids_all:
        ...
        if CREATE_IN_CRM and not crm_df.empty:
            created_ids, existing_ids = run_bulk_create_crm(crm_df)

            # 🔥 Update CRM flag
            full_df["is_crm_contact"] = full_df["apollo_person_id"].apply(
                lambda x: "true" if str(x) in created_ids or str(x) in existing_ids else "false"
            )

            # 🔥 Final update to master DB
            update_master_db_enriched(full_df)        
    ...
    if CREATE_IN_CRM:
        if crm_df.empty:
            ...
            # crm_df = run_bulk_create_crm(crm_df)
            created_ids, existing_ids = run_bulk_create_crm(crm_df)

            # 🔥 UPDATE FULL DF BASED ON CRM RESPONSE
            full_df["is_crm_contact"] = full_df["apollo_person_id"].apply(
                lambda x: "true" if str(x) in created_ids or str(x) in existing_ids else "false"
            )

            # 🔥 UPDATE MASTER DB AGAIN (FINAL STATE)
            update_master_db_enriched(full_df)

            # 🔥 STEP 1: Mark CRM contacts based on CRM Save input
            crm_person_ids = set(
                crm_df["apollo_person_id"].astype(str).str.strip()
            )

            full_df["is_crm_contact"] = full_df["apollo_person_id"].apply(
                lambda x: "true" if str(x).strip() in crm_person_ids else "false"
            )

            # Update timestamp
            # full_df.at[pid, "last_updated"] = TIMESTAMP
    ...
    # ───────────────────────────────────────────────
    # STEP 9: OPTIONAL SMART FIX (source_type based)
    # ───────────────────────────────────────────────
    full_df["is_crm_contact"] = full_df.apply(
        lambda row: "true" if row.get("source_type") == "CRM_CONTACT" else row["is_crm_contact"],
        axis=1
    )

    # ───────────────────────────────────────────────
    # STEP 10: FINAL MASTER DB UPDATE (AFTER CRM)
    # ───────────────────────────────────────────────
    update_master_db_enriched(full_df)
    ...
    print("  'Matched'   = Rows where Apollo API returned data (not guaranteed to have email))")
    print("  'Unmatched' = Rows Apollo could not find any match")
    print("  'CRM Save'  = Ready for CRM Upload with contacts/bulk_create API")
    print("─" * 62 + "\n")
