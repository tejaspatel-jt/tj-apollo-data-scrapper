...
═══════════════════════════════════════════════════════════════
  ...
  OUTPUT: output/apollo_people_search_DDMMMYYYY_HHMMSS.xlsx
          master_lead_database.xlsx (Maintains state across runs)
═══════════════════════════════════════════════════════════════
"""
...
def search_people(domain, company_name):
    ...
    while page <= MAX_PAGES:
        response = requests.post(
            ...
            json={"q_organization_domains_list": [domain], "person_titles": PERSON_TITLES},
...
def search_contacts(domain):
    ...
    while page <= MAX_PAGES:
        response = requests.post(
            ...
            json={"q_organization_domains_list": [domain], "person_titles": PERSON_TITLES, "page": page},
            headers={"x-api-key": API_KEY, "Content-Type": "application/json"},
            timeout=30
        )
        if response.status_code != 200: break
...
def flatten_person(person, company_name, domain, source_type="NEW_PROSPECT"):
    ...
    # If source is contact search, the ID is often 'person_id' inside the contact object
...
def main():
    ...
    # Load existing Master DB if it exists to maintain global state
    ...
    # Save the specific output for this run
