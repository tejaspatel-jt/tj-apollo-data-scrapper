...
BULK_ENRICH_INPUT_FILE = "apollo_people_search_08sep2026_120722.xlsx"
...
def load_input_file():
    ...
    if ext in (".xlsx", ".xls"):
        df = pd.read_excel(path, sheet_name=BULK_ENRICH_INPUT_SHEET)
    elif ext == ".csv":
        df = pd.read_csv(path)
    else:
    ...
    if ext in (".xlsx", ".xls"):
        print(f"  Loaded {len(df)} contacts from sheet '{BULK_ENRICH_INPUT_SHEET}' in 'output/{BULK_ENRICH_INPUT_FILE}'")
    else:
