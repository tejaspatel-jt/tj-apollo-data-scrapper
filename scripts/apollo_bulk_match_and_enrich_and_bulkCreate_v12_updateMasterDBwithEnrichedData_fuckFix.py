...
# LAST STEP: UPDATE MASTER DB WITH NEW ENRICHED DATA & STATUS ( Fun is updated in this version)
# v11 : no longer using this function, since we are now building the enriched rows with the final rules in save_output and then just merging with master DB there. This avoids having two separate loops over the data and ensures the master DB is updated with the exact same enriched data that we have in our output file.
def DONT_USED_update_master_db(input_df, enrich_map):
    ...
    for _, row in input_df.iterrows():
        ...
        # ── CASE 1: Enriched (API returned data) ─────────────────
        if pid in enrich_map:
        ...
        # ── CASE 2: No API result (unmatched) ─────────────────────
        else:
            record = {
                "apollo_person_id": pid,
                "first_name": "",
                ...
                # 🔥 IMPORTANT
                "is_enriched": "true",   # API was attempted
                "is_matched": "false",   # No data returned
...
def update_master_db_enriched(new_df):
    ...
    for pid, new_row in new_df.iterrows():
        ...
        if pid in master_df.index:
            ...
            # Update timestamp
            master_df.at[pid, "last_updated"] = pd.Timestamp.now().strftime("%Y-%m-%d %H:%M:%S")
...
def main():
    ...
    # ───────────────────────────────────────────────
    # STEP 9: Optional auto-save CRM Save
    # ───────────────────────────────────────────────
    if CREATE_IN_CRM:
        if crm_df.empty:
