...
def clean_input_value(val):
    """Treat placeholder values like NA/N/A/None/- as blank."""
    val = str(val).strip()
    return "" if val.lower() in NA_TOKENS else val
        ...
        return [
            {
                ...
                # Handles placeholder values like NA/N/A/None/- as blank and will also strip whitespace
                "org_industry_input": clean_input_value(row.get(industry_col, "")) if industry_col else "",
                "org_employees_input": clean_input_value(row.get(employees_col, "")) if employees_col else "",
            }
            for _, row in df.iterrows()
        ]
